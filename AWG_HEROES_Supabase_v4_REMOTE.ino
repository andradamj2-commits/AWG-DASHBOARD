...
// Poll for remote commands (relay/mode/stopall) from the dashboard
// every 3 seconds — this is how the web dashboard controls the
// system from anywhere, via the awg_commands table.
// Command-poll speed adapts to system mode: fast while Manual (you're
// actively tapping relay switches on the dashboard), slow while
// Automatic (the cycle runs itself — nothing to react to quickly).
#define SUPABASE_COMMANDS_POLL_INTERVAL_MANUAL 1000UL   // 1s — feels instant while controlling relays
#define SUPABASE_COMMANDS_POLL_INTERVAL_AUTO   12000UL  // 12s — automatic mode rarely needs a command applied fast
...
// ── Supabase Remote Command Polling ──────────────────────────────
// This is what makes the dashboard's controls work from anywhere,
// not just on the local WiFi. The web dashboard never talks to the
// ESP32 directly — it INSERTs a row into the awg_commands table in
// Supabase. The ESP32 polls that table on a timer, applies each
// pending command through the SAME safety checks used by the local
// /relay, /mode and /stopall endpoints, then marks the row processed
// so it isn't applied twice.
//
// Expected awg_commands columns (see the SQL you were given):
//   id             bigint, primary key
//   command_type   text   — "relay" | "mode" | "stopall"
//   relay_id       int    — 1-4, only for command_type = "relay"
//   state          text   — "on" | "off", only for command_type = "relay"
//   mode           text   — "auto" | "manual", only for command_type = "mode"
//   processed      boolean, default false

void applyRemoteRelayCommand(int id, bool on) {
  if (id < 1 || id > 4) return;

  // Same guards as the local /relay endpoint:
  if (g_systemMode == MODE_AUTOMATIC && id != 4) return;   // Auto owns relays 1-3
  if (id != 4 && on && g_noWaterDetected) return;          // no-water safety guard
  if (id == 3 && on && g_treatedTankFull) return;          // don't overfill treated tank

  setRelayState(id, on);
}

void applyRemoteModeCommand(const String& mode) {
  if (mode == "auto") {
...
void markCommandProcessed(long id) {
  ...
  String url = String(SUPABASE_URL) + "/rest/v1/awg_commands?id=eq." + String(id);
  if (!http.begin(client, url)) return;

  http.addHeader("Content-Type", "application/json");
  http.addHeader("apikey", SUPABASE_PUBLISHABLE_KEY);
  http.addHeader("Prefer", "return=minimal");
  http.sendRequest("PATCH", "{\"processed\":true}");
  http.end();
}
...
void pollSupabaseCommands() {
  ...
  String url = String(SUPABASE_URL) +
               "/rest/v1/awg_commands?processed=eq.false&order=id.asc&limit=5";
  ...
  if (err) {
    Serial.print("[SUPABASE] Command JSON parse error: ");
  ...
  for (JsonObject cmd : doc.as<JsonArray>()) {
    ...
    if (type == "relay") {
      int relayId  = cmd["relay_id"].as<int>();
      String state = cmd["state"].as<String>();
      applyRemoteRelayCommand(relayId, state == "on");
    } else if (type == "mode") {
      applyRemoteModeCommand(cmd["mode"].as<String>());
...
void loop() {
  ...
  // Check for remote commands from the dashboard. Poll fast (1s) in
  // Manual mode since you might tap a relay switch at any moment; poll
  // slow (12s) in Automatic mode since the cycle runs itself and rarely
  // needs a command applied quickly. Note this only affects how often we
  // CHECK for commands — sensor data upload above is unaffected and
  // always stays on its fixed 10s heartbeat regardless of mode.
  unsigned long commandPollInterval = (g_systemMode == MODE_MANUAL)
                                         ? SUPABASE_COMMANDS_POLL_INTERVAL_MANUAL
                                         : SUPABASE_COMMANDS_POLL_INTERVAL_AUTO;
